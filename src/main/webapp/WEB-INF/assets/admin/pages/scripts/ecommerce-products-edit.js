    ...
    var uploader = new plupload.Uploader({
      ...
      init: {
        PostInit: function() {
          ...
          $("#tab_images_uploader_filelist").on(
            "click",
            ".added-files .remove",
            function() {
              uploader.removeFile(
        ...
        UploadProgress: function(up, file) {
          $("#uploaded_file_" + file.id + " > .status").html(
            file.percent + "%"
          );
        },
        ...
        FileUploaded: function(up, file, response) {
          ...
          if (response.result && response.result == "OK") {
            ...
            $("#uploaded_file_" + file.id + " > .status")
              .removeClass("label-info")
              .addClass("label-success")
              .html('<i class="fa fa-check"></i> Done'); // set successfull upload
          } else {
            $("#uploaded_file_" + file.id + " > .status")
              .removeClass("label-info")
              .addClass("label-danger")
              .html('<i class="fa fa-warning"></i> Failed'); // set failed upload
            Metronic.alert({
              type: "danger",
